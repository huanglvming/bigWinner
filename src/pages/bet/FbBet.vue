<template>
	<div class="bet-wrapper">
		...
		<div class="banner football">
			<router-link to="/list/football" tag="div" class="goback"></router-link>
		...
		<div class="bet-content" v-else>
			<div class="bet-type" v-if="op">
				<div class="type-content">
					...
					<div class="bet-type-choose">
						<div class="choose-item" :class="{'already-bet':op_bets && op_support.win}" @click="itemClick($event,1)">
							<p class='item-title'>主队胜</p>
							<p class='item-roi'>赔率<span class='roi'>{{opObj.home_roi}}</span></p>
						</div>
						<div class="choose-item" :class="{'already-bet':op_bets && op_support.equal}" @click="itemClick($event,1)">
							<p class='item-title'>平</p>
							<p class='item-roi'>赔率<span class='roi'>{{opObj.draw_roi}}</span></p>
						</div>
						<div class="choose-item" :class="{'already-bet':op_bets && op_support.loose}" @click="itemClick($event,1)">
			...
			<div class="bet-type" v-if="jc">
				<div class="type-info" v-if="jcObj.handicap>0">—— 客队让{{jcObj.handicap | absolute}}球 ——</div>
				<div class="type-info" v-else>—— 主队让{{jcObj.handicap | absolute}}球 ——</div>
				<div class="type-content">
					<div class="bet-type-title-container">
						<div class="bet-type-title bet-handicap-wrapper">
							让球<p class="bet-handicap" v-if="jcObj.handicap>0">+{{jcObj.handicap}}</p>
							<p class="bet-handicap" v-else>{{jcObj.handicap}}</p>
						</div>
					</div>
					<div class="bet-type-choose">
						<div class="choose-item" :class="{'already-bet':jc_bets && jc_support.home}" @click="itemClick($event,2)">
							<p class='item-title'>主队胜</p>
							<p class='item-roi'>赔率<span class='roi'>{{jcObj.home_roi}}</span></p>
						</div>
						<div class="choose-item" :class="{'already-bet':jc_bets && jc_support.equal}" @click="itemClick($event,2)">
							<p class='item-title'>平</p>
							<p class='item-roi'>赔率<span class='roi'>{{jcObj.draw_roi}}</span></p>
						</div>
						<div class="choose-item" :class="{'already-bet':jc_bets && jc_support.guest}" @click="itemClick($event,2)">
							<p class='item-title'>客队胜</p>
			...
			<div class="bet-type" v-if="size">
				<div class="type-info">—— 全场比赛（不含加时，点球）两队得分总数 ——</div>
				<div class="type-content">
					<div class="bet-type-title-container">
						<div class="bet-type-title">
							大小球
					...
					<div class="bet-type-choose">
						<div class="choose-item" :class="{'already-bet':size_support.big}" @click="itemClick($event,4)">
							<p class='item-title'>大于{{sizeObj.handicap}}球</p>
							<p class='item-roi'>赔率<span class='roi'>{{sizeObj.big_roi}}</span></p>
						</div>
						<div class="choose-item" :class="{'already-bet':size_support.small}" @click="itemClick($event,4)">
							<p class='item-title'>小于{{sizeObj.handicap}}球</p>
			...
			<div class="bet-type" v-if="oddeven">
				...
				<div class="type-content">
					...
					<div class="bet-type-choose">
						<div class="choose-item" :class="{'already-bet':oddeven_bets && oddeven_support.odd}" @click="itemClick($event,5)">
							<p class='item-title'>单数</p>
							<p class='item-roi'>赔率<span class='roi'>{{odd_roi}}</span></p>
						</div>
						<div class="choose-item" :class="{'already-bet':oddeven_bets && oddeven_support.even}" @click="itemClick($event,5)">
...
<script>
	...
	export default{
		...
		watch:{
			bet_value(){
				if(parseInt(this.bet_value) === 500){
					jq(".bet-item").eq(0).addClass('bet-item-active').siblings().removeClass('bet-item-active');
				}else if(parseInt(this.bet_value) === 1000){
					jq(".bet-item").eq(1).addClass('bet-item-active').siblings().removeClass('bet-item-active');
				}else if(parseInt(this.bet_value) === 5000){
					jq(".bet-item").eq(2).addClass('bet-item-active').siblings().removeClass('bet-item-active');
				}else{
		...
		methods:{
			...
			itemClick(event,type){
				if(!jq(event.currentTarget).hasClass('already-bet') && this.status === 0){
					...
					if(type === 1){
						...
						this.r2 = $list.eq(1).find('.roi').text();
						this.r3 = $list.eq(2).find('.roi').text();
						switch(index){
							case 0:
								this.support = 0;
							...
							case 1:
								this.support = 2;
								this.order_text = '平局(赔率'+this.opObj.draw_roi+')';
								break;
							case 2:
								this.support = 1;
						...
						switch(index){
							case 0:
								...
								if(this.jcObj.handicap<0){
									this.order_text = this.homeTeam.name+'让'+Math.abs(this.jcObj.handicap)+'球 '+this.homeTeam.name+'胜(赔率'+this.jcObj.home_roi+')';
								}else{
									this.order_text = this.guestTeam.name+'让'+Math.abs(this.jcObj.handicap)+'球 '+this.homeTeam.name+'胜(赔率'+this.jcObj.home_roi+')';
								}
								break;
							case 1:
								this.support = 2;
								if(this.jcObj.handicap<0){
									this.order_text = this.homeTeam.name+'让'+Math.abs(this.jcObj.handicap)+'球 平局(赔率'+this.jcObj.home_roi+')';
								}else{
									this.order_text = this.guestTeam.name+'让'+Math.abs(this.jcObj.handicap)+'球 平局(赔率'+this.jcObj.home_roi+')';
								}
								break;
							case 2:
								this.support = 1;
								if(this.jcObj.handicap<0){
									this.order_text = this.homeTeam.name+'让'+Math.abs(this.jcObj.handicap)+'球 '+this.guestTeam.name+'胜(赔率'+this.jcObj.home_roi+')';
								}else{
									this.order_text = this.guestTeam.name+'让'+Math.abs(this.jcObj.handicap)+'球 '+this.guestTeam.name+'胜(赔率'+this.jcObj.home_roi+')';
								}
								break;
						}
					}else if(type === 4){
						...
						switch(index){
							case 0:
								this.support = 0;
								this.order_text = '得分总数大于'+this.sizeObj.handicap+'球(赔率'+this.sizeObj.big_roi+')';
								break;
							case 1:
								this.support = 1;
								this.order_text = '得分总数小于'+this.sizeObj.handicap+'球(赔率'+this.sizeObj.tiny_roi+')';
								break;
						}
					}else if(type === 5){
						this.r1 = $list.eq(0).find('.roi').text();
			...
			orderEvent(){
				let vm = this;
				if(parseInt(jq('.bet-number').val())>this.gold){
					this.tipsType = 0;
					this.mask = true;
				}else if(parseInt(jq('.bet-number').val())<100){
					vm.tipsType = 2;
					vm.mask = true;
				...
				}else if(!jq('.bet-number').val()){
					vm.tipsType = 2;
					vm.mask = true;
					vm.message = "请输入下单床币金额";
				}else if(this.isActive && parseInt(jq('.bet-number').val())>=100 && this.status === 0){
					jq.ajax({
						url: vm.baseURL + '/activity/contest/fb/bet',
						type: 'post',
						headers: {
							'ticket': localStorage.getItem('ticket')
						},
						...
						data: {
							...
							r2: this.r2,
							r3: this.r3,
							bet: parseInt(jq('.bet-number').val()),
							longbi: true
						},
						beforeSend: function(){
							vm.isActive = false;
